...
- **Order Hall: riga rimossa** (era `ohall`, il `classStart` del Demon Hunter, ora `t19`). Quei set non cadono dai boss, si comprano dal quartermaster uno slot alla volta. Tolta *per il momento*: se torna, serve anche una regola per lo slot, perché il gioco non gli dà `description` e finiva in `dropped` come «slot ignoto».
...
- **`armorType: true`** (BfA, Nathria/Sanctum, LFR di Hellfire Citadel) = set per tipo di armatura. Serve a **una cosa sola**: esentare la riga dal taglio di `classStart`, perché quei set non sono vincolati alla classe e un Evoker può portare il maglia di Uldir benché in BfA non esistesse. Sul sito non si vede nulla.
...
- **Boss nel tooltip: copertura parziale (83%),** cioè quante voci di `pieceList` hanno un boss fra parentesi (9046 su 10839). Per espansione: BfA/WoD 99%, Legion 98%, MoP 97%, SL 94%, TWW 78%, Midnight 77%, DF 75%, Cata 65%, Classic 52%, WotLK 43%, TBC 34%. Senza boss noto la voce resta il solo slot: **non inventarlo**.
...
- **Esclusi di proposito: i boss dei raid PvP da un boss solo** (Koralon e Toravon a Vault of Archavon, Argaloth e Occu'thar a Baradin Hold). Droppavano davvero guanti e gambali di T9–T12, e per T9/T10 erano l'unica fonte da boss esistente, ma droppano i pezzi di **tutte le classi** e indicarli non aiuta a farmare. Nei dati toccavano solo `Hands` e `Legs`: se ricompaiono su altri slot, quello è un errore vero.
