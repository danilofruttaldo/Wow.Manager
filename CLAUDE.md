...
- **Una riga = un set da collezionare**, non un raid. I raid senza set proprio non hanno una riga: o spariscono, o finiscono nel `name` della riga del set a cui contribuiscono (es. il T19 si chiama «The Emerald Nightmare · Trial of Valor · The Nighthold» perché il set viene dal Nighthold ma gli altri due appartengono allo stesso ciclo). Il codice regge ancora le righe con `versions` vuoto (riga tratteggiata, tooltip «Raid senza set collezionabile») ma **oggi non ne esiste nessuna**.
