...
- **`tiers`**: definizione delle righe, una volta sola per tutte le classi. Campi: `key, tier` (numero tier dove esiste, altrimenti la patch: `8.2`), `name` (raid), `exp`, `pieces` (pezzi per versione), `versions`, `note`, `warn`.
- **Ci sono TUTTI i raid, anche quelli senza niente da collezionare.** `versions` vuoto ⇒ riga tutta tratteggiata, tooltip «Raid senza set collezionabile» (Eye of Eternity, Ruby Sanctum, Baradin Hold, Emerald Nightmare, Sporefall…). Non cancellarle: servono a distinguere «raid senza set» da «raid dimenticato».
...
- **`classStart`**: primo raid in cui una classe esiste — DK `t7`, Monk `msv`, DH `ohall`, Evoker `t29`. Attenzione: è l'esistenza della **classe**, non del suo primo tier set (il DH esiste dall'inizio di Legion anche se il primo tier è il T19). Prima di quel raid l'intera riga è tratteggiata.
- **`collected`**: chiave = slug classe → `{ raid: { slot: pezzi_posseduti } }`. Slot mancante = 0. **È l'unica parte da aggiornare** man mano che si collezionano i pezzi; `tiers` cambia solo quando esce un raid nuovo.
