...
- **Boss nel tooltip: copertura parziale (80%),** cioè quante voci di `pieceList` hanno un boss fra parentesi (6401 su 8009). Per espansione: WoD/Legion 98%, MoP 90%, SL 84%, TWW 78%, Midnight 77%, DF 75%, WotLK 71%, Cata 70%, Classic 52%, TBC 34%. Senza boss noto la voce resta il solo slot: **non inventarlo**.
...
- **Sospetti ma non verificati: T14–T16** (Pandaria). Lì il boss varia per classe come nei moderni, ma a Pandaria i token erano davvero per *gruppo di classi* (Conqueror/Protector/Vanquisher), quindi la dispersione può essere legittima. Da controllare con una ricerca dedicata prima di toccarli. T17–T21 invece sono uniformi e corretti: non toccarli.
