...
- **`missing`** (blocco separato, come `collected`): `missing[classe][set][slot]` = elenco dei pezzi non ancora presi, es. `"Spalle (Ragnaros)"`. Alimenta il tooltip del contatore. **Va aggiornato insieme a `collected`**: li genera lo stesso dump ([scripts/transmog-tier-dump.lua](scripts/transmog-tier-dump.lua)) in due campi, `collectedJson` e `missingJson`. Se ne copi uno solo, il tooltip elenca pezzi già presi.
...
- **Boss nel tooltip: copertura parziale.** `GetAppearanceSourceDrops` risponde solo fino a Legion (MoP/WoD/Legion 89-96%, WotLK/Cata 29-47%, Classic/TBC 10-27%); da **Shadowlands in poi torna vuoto**. ⚠️ Non perché quei set non caschino dai boss — cascano, e Great Vault e Catalyst sono vie *aggiuntive*: è che dal T28 il boss droppa il **token**, e il pezzo di classe nasce dalla conversione, quindi l'apparenza dell'armatura non ha un drop associato. Il boss va cercato sul token (Encounter Journal), non sull'armatura. Senza boss noto la voce resta il solo slot: non inventarlo.
