...
function rosterBodyRows(rows: string[][], rowClass: string, header: string[]): { html: string; races: Set<string>; colCounts: number[] } {
  ...
  const html = rows.map((cells) => {
    ...
    // Totale PG della razza (riga): mostrato come numerino nella cella-testata, sotto l'icona.
    let rowTotal = 0;
    ...
    const tds = cells.map((cell, i) => {
      ...
      if (i === 0) {
        ...
        return `<td class="rhead">${label}${tot}</td>`;
      }
...
export function getRosterHtml(): string {
  ...
  // Riga Σ a piè di tabella: totale PG per classe + gran totale nell'angolo.
  const grand = colTotals.slice(1).reduce((a, b) => a + b, 0);
  const tfoot = '<tfoot><tr class="rtot-row">' + header.map((_, i) =>
    i === 0
      ? `<td class="rhead"><span class="rtot-sig" title="Totali per classe">Σ</span><span class="rtot rtot-grand" title="PG totali">${grand}</span></td>`
      : `<td>${colTotals[i] || ''}</td>`,
  ).join('') + '</tr></tfoot>';
